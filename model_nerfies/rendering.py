...
def render_samples(mlp, 
                   ...
                   z_vals, rays_d, use_white_background, sample_at_infinity, return_weights,
                   ):
    """
...
def volumetric_rendering(rgb, sigma, z_vals, dirs, use_white_background, sample_at_infinity, return_weights, eps=1e-10):
    ...
    dists = torch.concat([z_vals[..., 1:] - z_vals[..., :-1], torch.reshape([last_sample_z], z_vals[..., :1].size())], -1)
    dists = dists * torch.linalg.norm(dirs[..., None, :], axis=-1)
    alpha = 1.0 - torch.exp(-sigma * dists)
    # Prepend a 1.0 to make this an 'exclusive' cumprod as in `tf.math.cumprod`.
    accum_prod = torch.concat([
        torch.ones_like(alpha[..., :1], alpha.dtype),
        torch.cumprod(1.0 - alpha[..., :-1] + eps, axis=-1),
    ], axis=-1)
